...
$('searchButton').onclick = event => {
  ...
  input.id = 'insightsSearch'; input.type = 'search'; input.placeholder = 'Try collision, glass or rights';
...
function answerForPolicy(question) {
 ...
 if(/rent|transport|rental/.test(q))return {answer:'The 2018 Hyundai Elantra has Option 1 transportation expenses up to $900 per accident. A 48-hour total-theft or more-than-24-hour other-loss waiting period applies. No daily dollar rate is stated in the selected endorsement — this is the built-in transportation benefit, not a purchased rental-reimbursement product.',source:'rental'};
 if(/glass/.test(q))return {answer:'Glass breakage is listed as other than collision on this policy, and no full-glass line or glass-repair deductible waiver is established. A glass claim here would apply the standard $500 comprehensive deductible.',source:'physical'};
 if(/deduct|collision|comprehensive/.test(q))return {answer:'The Hyundai has $500 comprehensive and collision deductibles. UM property damage shows $200 in the declarations, while the endorsement qualifies that amount for unknown/hit-and-run vehicles. Review both with your insurance professional.',source:'physical'};
 if(/driver|insured|permissive|family member/.test(q))return {answer:'Tremayne Hedgepeth is the named insured and the application lists him as the Hyundai’s primary driver. Part A also extends liability to any permitted user of the vehicle — a broader group than the driver list shown on the declarations. Other driver-rating statuses are not stated.',source:'driver'};
 if(/premium|cost|prior term|renewal/.test(q))return {answer:'Listed coverage premiums total $2,098 for the six-month term, plus a $5 installment fee shown separately. This is a calculated sum; payment frequency is not stated, and a prior-term premium for renewal comparison is not in the reviewed pages.',source:'premium'};
 if(/rideshare|uber|lyft|tnc/.test(q))return {answer:'The policy excludes the specified liability and physical-damage losses during public/livery use, including time logged into a rideshare platform as a driver. No adding rideshare endorsement is listed. The documents do not establish active rideshare use.',source:'exclusions'};
 if(/med ?pay|medical|pip|injury/.test(q))return {answer:'Medical expense benefits are declined on this policy, at $0 premium — the Virginia amendment confirms Part B does not apply. Virginia is not a no-fault/PIP state, so a PIP component set is not expected here either. UM/UIM bodily injury remains at $100,000/$300,000.',source:'medical'};
 if(/right|entitle|clue|fcra/.test(q))return {answer:'Several statutory rights — like produce-the-policy requests, at-fault appeal windows and free CLUE reports — exist independently of what this policy purchased. Whether Virginia’s specific versions of these apply is not established in the reviewed pages; see Your rights on this policy.'};
 if(/lien|lease|gap|loan/.test(q))return {answer:'Santander is the listed lienholder for the Hyundai, but a loan/lease-payoff endorsement is not on this policy’s schedule, and no dealer GAP waiver is asserted. On a total loss with an outstanding balance above actual cash value, this policy alone may not cover the difference — ask your insurer.',source:'vehicle'};
