...
function showSource(id, trigger) {
  ...
  parts.splice(2,0,node('p','source-context','Source: supplied policy summary. The text below is a paraphrase, not a verified policy excerpt. Page numbers are reported references, not verified PDF page numbers.'));
  const unavailable=node('div','evidence-note boat-document-unavailable');unavailable.append(node('strong','','Original document unavailable'),node('p','','The policy PDF was not supplied with this summary.'));
  const link=node('a','','Read supplied summary');link.href='assets/policies/personal-boat-supplied-summary.txt';link.target='_blank';link.rel='noopener';unavailable.append(link);parts.push(unavailable);
...
$('searchButton').onclick = event => {
  ...
  input.id = 'insightsSearch'; input.type = 'search'; input.placeholder = 'Try towing or navigation';
  ...
  input.addEventListener('input', () => {
    ...
    status.textContent = matches.length ? `${matches.length} results` : 'No matching details. Try a coverage name such as towing.';
...
function answerForPolicy(question) {
 ...
 if(/deduct|hurricane|storm/.test(q))return {answer:'No actual deductible amounts are supplied. The summary describes separate deductibles by part and property item; any named-storm deductible needs the declarations.',source:'deductibles'};
 if(/navigat|territor|lay.up|canada/.test(q))return {answer:'Navigation boundaries, mooring and lay-up dates are not available. The summary mentions several territories and a breach correction provision, but the exact clause is needed to resolve its scope.',source:'navigation'};
 if(/tow|salvage|wreck/.test(q))return {answer:'The summary describes towing for breakdown or disablement without imminent danger and legally required wreck removal. Selected limits and approval conditions are not available.',source:'assistance'};
 if(/premium|cost|date|expir|policy number|carrier/.test(q))return {answer:'The declarations are not supplied. Premium, dates, policy number and underwriting company are unavailable.',source:'identity'};
 if(/crew|commercial|racing|charter/.test(q))return {answer:'The summary reports paid-crew injury, commercial-use, racing and operator restrictions. It does not identify actual crew, operators or vessel use. Check the issued wording and endorsements.',source:'use'};
 if(/valu|depreci|settle/.test(q))return {answer:'Two length-dependent endorsement variants are referenced. Vessel length, equipment age and the issued endorsement are needed to establish the settlement basis. No scheduled value is provided.',source:'endorsements'};
 return {answer:'This local preview has no verified answer for that question. Review the supplied summary with your agent or insurer; the original policy PDF and declarations are unavailable.'};
}
...
$('askOvieForm').onsubmit = event => {
  ...
  $('askOvieAnswer').textContent=`Sample answer based on the supplied summary.\n\n${answer}`;
...
document.addEventListener('click',event=>{
 if(event.target.closest('[data-professional]')){
  ...
  $('utilityBody').replaceChildren(node('p','source-context','Ask your agent or insurer to confirm the declarations, vessel schedule, navigation and lay-up conditions, and issued endorsements.'),node('p','evidence-note','No agent, insurer or claims contact is identified in the supplied summary.'));
