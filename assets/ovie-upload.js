(() => {
  ...
  function review(){
    ...
    main.innerHTML=`<section class="current-progress"><p class="muted">Preview your files and remove anything you don’t want to upload.</p></section>
      ...
      ${files.length?`<ul class="upload-review-list">${files.map(file=>`<li class="upload-review-row"><button class="upload-review-preview" data-preview-policy="draft" data-preview-index="${file.index}" data-preview-name="${esc(file.name)}" aria-label="Preview ${esc(file.name)}"><span class="upload-document-icon">${icon('file')}</span><span class="stack grow"><strong>${esc(file.name)}</strong><span class="caption">${esc(file.name.split('.').pop().toUpperCase())} · ${size(file.size)}</span></span></button><div class="upload-review-actions"><button class="icon-button clear" data-remove-file="${file.index}" aria-label="Remove ${esc(file.name)}">${icon('trash')}</button></div></li>`).join('')}</ul>`:`<p class="muted review-empty">${draft?'No files selected. Add files to continue.':'Choose files to review before uploading.'}</p>`}</section>
