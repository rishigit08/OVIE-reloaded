(() => {
  ...
  const icons = {
    trash:'<path d="M3 6h18M19 6v14a1 1 0 0 1-1 1H6a1 1 0 0 1-1-1V6M8 6V4a1 1 0 0 1 1-1h6a1 1 0 0 1 1 1v2M10 10v7M14 10v7"/>',
  ...
  if(['#review','#upload'].includes(location.hash)&&!localStorage.getItem(KEY))state.playing=false;
  ...
  const noInsights=()=>state.scenario==='unmatched'||state.scenario==='duplicate';
  ...
  const insightPolicies=()=>noInsights()||state.scenario==='uploadFailed'?[]:state.custom?['local']:state.scenario==='existingOnly'||state.scenario==='duplicate'?['umbrella']:state.scenario==='standalone'?['standalone']:state.scenario==='inbox'?['condo']:['condo','umbrella','standalone'];
  ...
  function policyGroup(name,meta,files,policy,added){
    ...
    const category=policy==='local'?'':added?'Existing policy':'New policy';
  ...
  async function previewDocument(button){
    ...
    const reviewing=button.dataset.previewPolicy==='draft';
    if(!reviewing&&!state.custom){body.innerHTML='<div class="document-preview-placeholder"><p class="pill category-badge">Demo document</p><p>This filename is part of the sample upload. Choose a PDF or image from your device to preview the original document here.</p></div>';return}
    try{
      const file=await flow.readFile(reviewing?new URLSearchParams(location.search).get('draft'):state.batchId,Number(button.dataset.previewIndex));
  ...
  function outcomes(){
    ...
    if(state.custom)return `<section class="card upload-file-card">${policyGroup('Uploaded documents','',state.custom,'local',false)}</section>`;
  ...
  let reviewFooterObserver;
  function review(){
    reviewFooterObserver?.disconnect();
    header.innerHTML=headerHtml('Review files','files',true);
    const id=new URLSearchParams(location.search).get('draft'),draft=flow.readDraft(id),files=draft?.files||[];
    const size=bytes=>bytes>=1048576?`${(bytes/1048576).toFixed(1)} MB`:`${Math.max(1,Math.round(bytes/1024))} KB`;
    main.innerHTML=`<section class="current-progress"><p class="muted">Preview your files and remove anything you don’t want to upload.</p></section>
      <section class="card upload-review-card" aria-label="Selected files"><div class="upload-review-heading"><h2>${files.length} ${files.length===1?'file':'files'} selected</h2><button class="text-action" data-upload-options>Add files</button></div>
      ${files.length?`<ul class="upload-review-list">${files.map(file=>`<li class="upload-review-row"><span class="upload-document-icon">${icon('file')}</span><div class="stack grow"><strong>${esc(file.name)}</strong><span class="caption">${esc(file.name.split('.').pop().toUpperCase())} · ${size(file.size)}</span></div><div class="upload-review-actions"><button class="icon-button clear" data-preview-policy="draft" data-preview-index="${file.index}" data-preview-name="${esc(file.name)}" aria-label="Preview ${esc(file.name)}">${icon('eye')}</button><button class="icon-button clear" data-remove-file="${file.index}" aria-label="Remove ${esc(file.name)}">${icon('trash')}</button></div></li>`).join('')}</ul>`:`<p class="muted review-empty">${draft?'No files selected. Add files to continue.':'Choose files to review before uploading.'}</p>`}</section>
      <div class="upload-review-confirmation"><p class="error-copy" id="review-error" role="alert"></p><button class="button" id="confirm-upload" ${files.length?'':'disabled'}>Confirm & generate insights</button></div>`;
    reviewFooterObserver=new ResizeObserver(([entry])=>{main.style.paddingBottom=`${entry.target.getBoundingClientRect().height+24}px`});
    reviewFooterObserver.observe(main.querySelector('.upload-review-confirmation'));
    main.querySelectorAll('[data-remove-file]').forEach(button=>button.onclick=()=>{
      const index=Number(button.dataset.removeFile),position=draft.files.findIndex(file=>file.index===index),removed=draft.files[position];
      const next={...draft,files:draft.files.filter(file=>file.index!==index)};
      try{flow.saveDraft(next);review();const buttons=main.querySelectorAll('[data-remove-file]');(buttons[Math.min(position,buttons.length-1)]||main.querySelector('[data-upload-options]')).focus({preventScroll:true});document.querySelector('#announcer').textContent=`${removed.name} removed from this upload.`}
      catch{main.querySelector('#review-error').textContent='The selection could not be updated. Please try again.'}
    });
    main.querySelector('#confirm-upload').onclick=async()=>{
      main.querySelectorAll('button').forEach(button=>button.disabled=true);
      const confirm=main.querySelector('#confirm-upload');confirm.textContent='Confirming…';
      try{
        state=await flow.confirmDraft(id);
        const url=new URL(location.href);url.searchParams.delete('draft');url.searchParams.delete('preview');url.searchParams.set('batch',state.batchId);url.hash='detail';
        history.replaceState(null,'',url);render(true);
      }catch(error){main.querySelectorAll('button').forEach(button=>button.disabled=false);confirm.textContent='Confirm & generate insights';main.querySelector('#review-error').textContent=error.message||'The upload could not be started. Please try again.'}
    };
  }
  function upload(){
    header.innerHTML=headerHtml('Choose files');
    main.innerHTML=`<section class="card"><h2>Add your documents</h2><p class="muted">Choose your files, then review them before confirming your upload.</p><button class="button" data-upload-options>Choose files</button><label for="picker" class="text-action">Select from device</label><input class="file-select" id="picker" type="file" accept=".pdf,.png,.jpg,.jpeg" multiple><p class="caption">PDF, PNG or JPG · Up to 10 files<br>25 MiB per file · 100 MiB per batch</p><p id="validation" class="error-copy" role="alert"></p></section>`;
    main.querySelector('#picker').onchange=async event=>{
      const files=Array.from(event.target.files);if(!files.length)return;
      try{const draft=await flow.createDraft(files);const url=new URL(location.href);url.searchParams.delete('preview');url.searchParams.set('draft',draft.id);url.hash='review';location.href=url.href}
      catch(error){main.querySelector('#validation').textContent=error.message||'These files could not be prepared. Please try again.'}
    };
  }
  ...
  function render(focus=false){reviewFooterObserver?.disconnect();main.style.paddingBottom='';let route=location.hash.slice(1)||'files';if(route==='files'){location.replace(returnPage);return}if(route==='activity'){route='detail';history.replaceState(null,'',location.pathname+location.search+'#detail')}nav.hidden=route!=='files';main.className=route==='files'?'with-nav':route==='review'?'upload-review':'';document.querySelector('#preview-options').hidden=route==='review';if(route==='files')files();else if(route==='detail')detail();else if(route==='upload')upload();else if(route==='review')review();else if(route==='standalone-insights')standaloneInsights();else if(route.startsWith('policy/'))policy(route.split('/')[1]);else inboxInfo();document.querySelector('#advance').disabled=terminal();document.querySelector('#advance').textContent=flow.elapsed(state)<6000?'Finish reading data':'Complete next policy';document.querySelector('#play').disabled=terminal();document.querySelector('#play').textContent=state.playing?'Pause preview':'Play process';document.querySelectorAll('[name=scenario]').forEach(r=>r.checked=r.value===state.scenario);window.dispatchEvent(new Event('ovie-upload-preview-changed'));if(focus){if(restoreIndicator){document.querySelector('[data-upload-entry]')?.focus();restoreIndicator=false}else (header.querySelector('h1')||main.querySelector('h1'))?.focus({preventScroll:true})}}
  ...
  if(!['#review','#upload'].includes(location.hash)&&scenarios.some(([id])=>id===previewStage))selectPreview(previewStage);else{if(!['#review','#upload'].includes(location.hash))save();render(location.hash==='#review')}
})();
